...
def proceed_to_checkout():
    ...
    try:
        ...
        app.deiconify()  # Make sure app is visible if there was an error
...
def refresh_previous_orders():
    ...
    for order in orders:
        ...
        view_btn = ctk.CTkButton(order_row, text="View", 
                               ...
                               font=("Arial", 12), width=80, height=30,
                               command=lambda order_id=order['order_id']: view_order_details(order_id))
...
# Function to refresh products display
def refresh_products_display(search_query=None):
...
def handle_search():
    ...
    refresh_products_display(search_query)
...
# Search bar
search_frame = ctk.CTkFrame(items_section, fg_color="white")
search_frame.pack(fill="x", pady=(0, 10))

search_entry = ctk.CTkEntry(search_frame, placeholder_text="Search products...", 
...
# Fetch products from database and display them
...
# If a command-line argument "cart" is provided, show cart view
if len(sys.argv) > 2 and sys.argv[2] == "cart":
    show_cart_view()
